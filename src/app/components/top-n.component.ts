  ...
  ngOnInit() {
    ...
    this.emojiUpdatesNotifySubject = this.emojiTrackerService
      ...
      .subscribe((data: EmojiData) => {
        console.log(`\n---------------------------------------`);
        console.log(`[Top-N] Update by emoji (${data.emoji}) Object-Count: ${this.emojiDataObjects.length}`);
        ...
        if (i > -1) {
          if (this.emojiDataObjects[i].count !== data.count) {
            console.log(`[Top-N] Will Update, Old count: ${this.emojiDataObjects[i].count} / New count: ${data.count}`);
            this.emojiDataObjects[i] = {...data, updated: true};
            console.log(`[Top-N] Updated index (${i}), count: ${this.emojiDataObjects[i].count}, emoji: ${this.emojiDataObjects[i].emoji}`);
            this.ref.detectChanges();
            this.emojiDataObjects = this.emojiDataObjects.sort(
              (a: EmojiData, b: EmojiData) => b.count - a.count
            );
            console.log(`[Top-N] Object-Count after sorting: ${this.emojiDataObjects.length}`);
            console.log(`[Top-N] Begin animation of ${this.emojiDataObjects[i].emoji}`);
            setTimeout(() => {
              console.log(`[Top-N] End animation of ${this.emojiDataObjects[i].emoji}`);
              this.emojiDataObjects[i].updated = false;
              this.ref.detectChanges();
              console.log(`[Top-N] Object-Count after animation: ${this.emojiDataObjects.length}`);
              console.log(`---------------------------------------\n`);
            }, 500);
          }
        } else {
